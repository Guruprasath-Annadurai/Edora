...
--
-- Includes a REAL-ROLE test with SET ROLE anon / authenticated / service_role.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Test execution as authenticated user
  ...
  -- Idempotency check: repeat call must be duplicate
  v_res := public.complete_quiz_session(
    v_sid, 'Physics', 'Optics', '[]'::jsonb, '[]'::jsonb, 5, 5, now()
  );
...
BEGIN
  ...
  -- Check permissions: anon and authenticated can SELECT, not write
...
-- Insert test questions across all academic states
INSERT INTO public.pyq_content (question_text, is_active, is_reviewed, flagged_for_review, reviewed_by)
VALUES
  ('Q1: Valid AI reviewed question',     true,  true,  false, 'ai:gpt-oss-120b'),
  ('Q2: Valid Human verified question',  true,  true,  false, 'human:prof_sharma'),
