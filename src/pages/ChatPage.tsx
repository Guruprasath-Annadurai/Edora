...
const SYSTEM_PROMPTS = {
  teacher: 'You are Nova, an expert AI tutor in teacher mode. Give structured, clear explanations with examples. Break complex topics into digestible steps.',
  friend: 'You are Nova, a friendly study buddy. Keep it casual and encouraging. Use simple language and relate topics to everyday life.',
};
    ...
    try {
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${import.meta.env.VITE_GEMINI_API_KEY}`, {
        ...
        body: JSON.stringify({
          ...
          contents: [
            ...messages.slice(-6).map(m => ({ role: m.role === 'user' ? 'user' : 'model', parts: [{ text: m.content }] })),
      ...
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', content: "Connection issue. Please try again.", timestamp: new Date() }]);
    } finally {
      setLoading(false);
    }
  }
          ...
          {messages.map(msg => (
            <motion.div key={msg.id}
              ...
              <div className={`max-w-[78%] px-4 py-3 rounded-2xl text-sm leading-relaxed
                ...
                style={msg.role === 'user' ? { background: 'linear-gradient(135deg, #7C3AED, #3B82F6)' } : {}}>
                {msg.content}
              </div>
            </motion.div>
          ))}
