      ...
      <motion.div
        ...
        <Link to="/novo-insights" className="block active:scale-[0.98] transition-transform">
          <div className="rounded-3xl overflow-hidden relative"
            style={{
              background: 'linear-gradient(135deg, var(--grad-purple-header-1) 0%, var(--grad-purple-header-2) 45%, var(--grad-purple-header-3) 100%)',
            ...
            <div className="absolute inset-0 opacity-10"
              style={{
                backgroundImage: 'radial-gradient(circle at 20% 50%, var(--ink-150) 0%, transparent 60%), radial-gradient(circle at 80% 20%, var(--ink-100) 0%, transparent 50%)',
              }} />
            ...
            <div className="relative px-4 py-4">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {weak.map(s => (
                  <div key={s.subject} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl"
                    ...
                    <span className="text-xs font-semibold text-red-200">{s.subject}</span>
                    <span className="text-xs text-red-300/70">{s.score_pct}%</span>
                  </div>
                ))}
                {strong.map(s => (
                  <div key={s.subject} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl"
                    ...
                    <span className="text-xs font-semibold text-emerald-200">{s.subject}</span>
                    <span className="text-xs text-emerald-300/70">{s.score_pct}%</span>
                  </div>
                ))}
                {/* XP pill */}
                <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl"
                  style={{ background: 'rgba(251,191,36,0.15)', border: '1px solid rgba(251,191,36,0.25)' }}>
                  <span className="text-xs font-semibold text-yellow-300">+{insight.xp_this_week} XP</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 px-3 py-1.5 rounded-xl"
                  style={{ background: 'var(--ink-120)' }}>
                  <span className="text-xs font-bold text-white">View full report</span>
