...
- **NEW FINDING while wiring CI (§16 below):** `npm audit --omit=dev` (production-scope only) shows **8 vulnerabilities (4 moderate, 4 high), 0 critical** — the critical `tar` finding is confirmed entirely dev-scoped, consistent with the analysis above. But production scope surfaced findings not previously flagged: `@huggingface/transformers` (high, direct dependency, `range: *` — i.e. all published versions affected) via its `onnxruntime-node`/`adm-zip`/`sharp` dependency chain, used in `src/workers/offlineModel.worker.ts` (the offline/on-device AI fallback — this is real shipped-client code, not a build tool), plus `react-router`/`react-router-dom` (moderate — open-redirect and SSR-hydration constructor-injection advisories; this app doesn't use SSR, so the second is likely inapplicable, but not yet confirmed). **None of these have been triaged or fixed — flagged here as NOT STARTED, out of scope for the "wire audit into CI" task that surfaced them.**
