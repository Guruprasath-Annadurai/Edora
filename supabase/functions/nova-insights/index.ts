// ═══════════════════════════════════════════════════════════════
// Edora — Nova Insights Edge Function
//
...
//   3. Upserts nova_insights row
//   4. Sends FCM push notification (if FIREBASE_SERVER_KEY is set)
//
// Deploy:  supabase functions deploy nova-insights
...
async function callGemini(
  dataSummary: string,
  apiKey: string,
): Promise<NovaInsightPayload> {
  const systemPrompt = `You are Nova, the AI study coach inside the Edora learning app.
...
- motivation: 1–2 sentences, personal and encouraging.`;
  ...
  const userPrompt = `Generate a Nova Insights report for this student:\n\n${dataSummary}`;
  ...
  try {
    ...
    return JSON.parse(raw) as NovaInsightPayload;
  } finally {
  ...
  try {
    ...
    for (const profile of (profiles as ProfileRow[])) {
      try {
        ...
        if (upsertErr) {
          console.error(`[Nova] upsert error for ${profile.id}:`, upsertErr.message);
          errors++;
          continue;
        }
        ...
        console.error(`[Nova] error for user ${profile.id}:`,
    ...
    if (pushTokens.length > 0 && firebaseServerKey) {
      await sendFcmBatch(
        pushTokens,
        '✨ Your Nova Insights are ready',
        'See your personalised study plan for this week',
        firebaseServerKey,
      );
      console.log(`[Nova] FCM push sent to ${pushTokens.length} device(s)`);
    } else if (!firebaseServerKey) {
      console.log('[Nova] FIREBASE_SERVER_KEY not set — skipping push notifications');
    }

    const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
    console.log(`[Nova] Done: ${processed} succeeded, ${errors} failed in ${elapsed}s`);
    ...
    console.error('[Nova] Fatal error:', err instanceof Error ? err.message : String(err));
    return new Response(
