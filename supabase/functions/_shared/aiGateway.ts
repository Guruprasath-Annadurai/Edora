...
const COST_PER_1M_TOKENS_USD: Record<string, { input: number; output: number }> = {
  ...
  // Anthropic — added 2026-08-28 for gemini-chat's feature-flagged Claude
  // path (AI_CHAT_PROVIDER=claude). Model id strings match what's actually
  // called in gemini-chat/index.ts (CLAUDE_MODEL_PRIMARY/FALLBACK) -- keep
  // these in sync if those change. Rates are estimates, not fetched live;
  // check console.anthropic.com/settings/billing if the ceiling looks off.
