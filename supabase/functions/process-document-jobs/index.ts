...
async function generatePack(text: string, fileName: string, apiKey: string): Promise<StudyPack> {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      const candidate = JSON.parse(cleaned) as Partial<StudyPack>;

      if (!isValidStudyPack(candidate)) {
        lastErr = 'Invalid study pack structure from Gemini (wrong item count or missing required fields)';
        continue;
      }
      return candidate;
    } catch (e) {
