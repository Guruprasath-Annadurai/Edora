// ─────────────────────────────────────────────────────────────────────────────
// exam-prediction — projects likely exam score + minimum daily effort needed
// Actions: predict | get_cached | clear_cache
// ─────────────────────────────────────────────────────────────────────────────
...
serve(withSentry('exam-prediction', async (req) => {
  ...
  if (action === 'predict') {
    ...
    const [
      ...
      supabase.from('sr_cards').select('subject,topic,ef_factor,repetitions,last_reviewed_at,next_review_date').eq('user_id', user.id),
      supabase.from('user_challenge_attempts').select('score,subject,challenge_date').eq('user_id', user.id).eq('status', 'completed').order('challenge_date', { ascending: false }).limit(30),
      supabase.from('user_topic_progress').select('status, curriculum_topics(title, subject)').eq('user_id', user.id),
      supabase.from('sprint_sessions').select('xp_earned,completed,created_at').eq('user_id', user.id).order('created_at', { ascending: false }).limit(30),
      supabase.from('confidence_events').select('subject,topic,confidence_score').eq('user_id', user.id).order('created_at', { ascending: false }).limit(100),
    ]);

    // Compute days remaining (null when no exam date → readiness-mode)
    ...
    for (const sub of subjects) {
      ...
      s.weak_topics = [...new Set(s.weak_topics)]; // deduplicate
    }
    ...
    const sprintsPerWeek = completedSprints / 4; // last 30 days → ~4 weeks
    ...
    const prediction = await geminiJSON<PredictionResult>(`
...
Target: ${target_grade ? `Grade ${target_grade}` : target_score ? `${target_score}%` : 'Not specified'}

Based on this data:
1. Estimate the student's current exam-readiness score (what they would score today)
...
Return JSON:
{
  ...
  "target_score": <integer 0-100, use ${target_score ?? 75} if not specified>,
