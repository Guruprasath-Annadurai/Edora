-- ═══════════════════════════════════════════════════════════════════════════
-- B2B2C Institutional Layer — School Admin Portal
-- Institution onboarding, bulk student management, principal analytics
-- ═══════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.institutions (
  ...
  -- Pricing: free=50 students, starter=₹999/mo 200 students, pro=₹4999/mo unlimited
  max_students    INTEGER     NOT NULL DEFAULT 50,
  join_code       TEXT        UNIQUE NOT NULL,      -- 8-char code for bulk student join
  join_link_token TEXT        UNIQUE NOT NULL,      -- UUID token for QR link
  admin_user_id   UUID        NOT NULL REFERENCES public.profiles(id) ON DELETE RESTRICT,
  logo_url        TEXT,
  is_verified     BOOLEAN     NOT NULL DEFAULT false,
  student_count   INTEGER     NOT NULL DEFAULT 0,  -- cached, updated by trigger
...
-- Admin can see their own institution
...
-- Members can read their institution
...
CREATE TABLE IF NOT EXISTS public.institution_members (
  ...
  class_section   TEXT,       -- e.g. '11A', '12 Science'
...
-- Allow authenticated users to join via code (INSERT only, own user_id)
...
-- ── 3. Update student_count on member change ──────────────────────────────────
...
-- ── 6. Create institution (admin self-service) ────────────────────────────────
...
BEGIN
  ...
  -- Prevent duplicate institution per admin
...
-- ── 7. School analytics aggregation view ─────────────────────────────────────
...
-- ── 8. Per-student analytics for principal dashboard ─────────────────────────
...
-- ── 9. Weak-topic class summary (for teacher/admin) ──────────────────────────
...
-- ── 10. Referral reward: streak freeze on successful referral ──────────────────
-- Add streak_freeze_count to profiles if not already there
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS streak_freeze_count INTEGER NOT NULL DEFAULT 0;

-- Update referral_rewards to include streak freeze bonuses
...
-- Update process_referral to award streak freezes
...
-- ── 11. Rank snapshots for push notifications ─────────────────────────────────
-- Function to insert daily rank snapshots (called by cron)
...
-- ── 12. Push notification tracking ───────────────────────────────────────────
-- Track which push types were sent to avoid spam
CREATE TABLE IF NOT EXISTS public.push_log (
  id           UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id      UUID        NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  push_type    TEXT        NOT NULL,  -- 'streak','rank_drop','daily_q','referral','challenge'
...
-- Cleanup old push logs (keep 30 days)
