...
drop policy if exists "buddies_service_write" on "study_buddies";
drop policy if exists "buddies_service_insert" on "study_buddies";
create policy "buddies_service_insert" on "study_buddies" for insert with check (auth.role() = 'service_role');
drop policy if exists "buddies_service_update" on "study_buddies";
...
drop policy if exists "buddies_service_delete" on "study_buddies";
create policy "buddies_service_delete" on "study_buddies" for delete using (auth.role() = 'service_role');
drop policy if exists "buddies_participants" on "study_buddies";
drop policy if exists "buddies_read" on "study_buddies";
...
drop policy if exists "sgm_all" on "study_group_members";
drop policy if exists "sgm_read_peers" on "study_group_members";
drop policy if exists "sgm_insert" on "study_group_members";
...
drop policy if exists "ta_teacher_manage" on "teacher_assignments";
drop policy if exists "ta_student_read" on "teacher_assignments";
drop policy if exists "ta_teacher_insert" on "teacher_assignments";
create policy "ta_teacher_insert" on "teacher_assignments" for insert with check ((select auth.uid()) = teacher_id);
drop policy if exists "ta_teacher_update" on "teacher_assignments";
...
drop policy if exists "ta_teacher_delete" on "teacher_assignments";
...
drop policy if exists "tp_update" on "tournament_participants";
...
drop policy if exists "vqb_admin_write" on "verified_question_bank";
drop policy if exists "vqb_admin_insert" on "verified_question_bank";
...
drop policy if exists "vqb_admin_update" on "verified_question_bank";
...
drop policy if exists "vqb_admin_delete" on "verified_question_bank";
create policy "vqb_admin_delete" on "verified_question_bank" for delete using (
  (exists (select 1 from user_roles ur where ur.user_id = (select auth.uid()) and ur.role = any (array['admin','moderator']::app_role[])))
  or (auth.role() = 'service_role')
);
drop policy if exists "vqb_read_approved" on "verified_question_bank";
drop policy if exists "vqb_read" on "verified_question_bank";
